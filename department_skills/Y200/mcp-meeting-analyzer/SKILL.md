...
version: "1.0.0"
description: >
  會議逐字稿結構化分析工具。將會議逐字稿經過 AI 處理 （文本清洗→人員歸戶），產出乾淨書面文本與組織識別資料。 輸出可直接回覆使用者，或搭配 mcp-notion-crud 寫入 Notion。 當使用者提到「整理會議紀錄」「摘要逐字稿」「會議重點」 「歸納會議內容」「整理這段文字」「會議摘要」「meeting summary」 「幫我整理會議」「分析會議內容」時觸發此技能。 【重要】呼叫此技能時，必須將使用者訊息中的完整逐字稿或會議記錄文字 作為 transcript 參數傳入，不可省略或留空。
