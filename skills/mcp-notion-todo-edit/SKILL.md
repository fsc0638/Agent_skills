...
display_name: "編輯或刪除 Notion ToDo 項目"
provider: mcp
version: "1.0.0"
description: >
  編輯或刪除 Notion ToDo 項目。支援更新狀態、負責人、到期日、專案等欄位， 以及封存（軟刪除）指定項目。需搭配 mcp-notion-query 先查詢取得 page_id。 當使用者提到「更新待辦」「改狀態」「標記完成」「修改todo」「刪除任務」 「移除待辦」「刪掉那筆」「把...改成」時觸發此技能。
runtime_requirements: [requests]
risk_level: high
risk_description: >
  此技能會修改或封存 Notion 資料庫中的頁面，屬於外部系統寫入操作。 更新操作會覆寫指定欄位，刪除操作會將頁面封存（可在 Notion 還原）。
recommended_models:
  openai: gpt-4.1-nano
